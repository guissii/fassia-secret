    ...
    try {
      ...
      for (const collName of collections) {
        ...
        const collection = await prisma.collection.upsert({
          where: { slug: collSlug },
          update: {},
          create: { name: collName, slug: collSlug, description: `Collection ${collName}`, page: 'complements-alimentaires', order: 0 },
        });
      ...
      if (steps.length > 0) {
        const kbColl = await prisma.collection.upsert({
          ...
          update: {},
          create: { name: 'Korean Beauty', slug: 'korean-beauty', description: 'Routine Korean Beauty', page: 'korean-beauty', order: 0 },
        });
